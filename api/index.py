...
# --- your agent imports (keep these as you had them) ---
from agents import (
    function_tool,
    Runner,
    Agent,
...
# ---------- Tools & helper functions (scrapers, etc.) ----------
# (I copied your tool implementations unchanged so your Agent can use them)
# Keep all your @function_tool decorated functions here (scrap_full_text, scrap_headings, ...)
...
async def scrap_meta(site: str) -> dict:
    try:
        ...
        if meta_tag and meta_tag.get('content'):
            meta_description = meta_tag['content'].strip()
        return {"title": title, "description": meta_description}
    except Exception as e:
        return {"error": f"An error occurred: {str(e)}"}
...
async def scrap_og_and_verification(site: str) -> dict:
    try:
        async with httpx.AsyncClient() as client:
            ...
            response.raise_for_status()
        soup = BeautifulSoup(response.text, 'html.parser')
        result = {"og_tags": {}, "google_site_verification": None, "google_tag_manager": []}
        for tag in soup.find_all("meta"):
            prop = tag.get("property")
            if prop and prop.startswith("og:"):
                content = tag.get("content", "").strip()
...
async def check_site_protocol_ssl(domain: str) -> dict:
    domain = domain.strip().replace("https://", "").replace("http://", "").replace("/", "")
    result = {"www_version": None, "non_www_version": None, "preferred_url": None, "protocol": None, "ssl_expiry": None}
    async def try_url(url):
        try:
            async with httpx.AsyncClient(follow_redirects=True, timeout=5) as client:
                r = await client.get(url)
    ...
    result["non_www_version"] = non_www_works
    if www_works:
        result["preferred_url"] = www_url; result["protocol"] = "https"
    elif non_www_works:
        result["preferred_url"] = non_www_url; result["protocol"] = "https"
    else:
        www_http = f"http://www.{domain}"; non_www_http = f"http://{domain}"
        if await try_url(www_http):
            result["preferred_url"] = www_http; result["protocol"] = "http"
        elif await try_url(non_www_http):
            result["preferred_url"] = non_www_http; result["protocol"] = "http"
        else:
            result["preferred_url"] = None; result["protocol"] = None
            result["ssl_expiry"] = "Could not determine - site not reachable"
            return result
    if result["protocol"] == "https":
        try:
    ...
    try:
        ...
        for img in images:
            ...
            if alt:
                ...
                key = f'img{fallback_count}'; fallback_count += 1
            result[key] = src
        return result
    except Exception as e:
        return {"error": f"An error occurred: {str(e)}"}

# (You can keep get_all_pages_classified function as in your original file if needed.)
# For brevity I assume you will copy/paste your full get_all_pages_classified implementation here.

# ---------- Setup Gemini/OpenAI client for Agent ----------
# Use AsyncOpenAI wrapper you referenced; supply the GEMINI base + api key.
if not API_KEY:
    logger.warning("API_KEY not set in environment. Agent calls will fail until API_KEY is provided.")

# Create AsyncOpenAI client pointing to Google generative endpoint that mimics OpenAI API shape
client = AsyncOpenAI(
    base_url=f"{GEMINI_BASE}/v1beta/openai/",
    api_key=API_KEY
)

# Global Agent config (as you had)
MODEL = MODEL  # already defined
set_default_openai_api('chat_completions')
set_default_openai_client(client=client)
set_tracing_disabled(True)

# Build the Agent (copy your whole Agent(...) block here — I preserved it exactly)
agent = Agent(
    name="SEO Agent",
    instructions="""
    ## System Prompt
    ...
    model=MODEL,
    tools=[
        # tools refer to the function_tool functions you defined above
        # ensure get_all_pages_classified is defined earlier in this file if you need it
        # include only the tools you implemented above
        # Example:
        # get_all_pages_classified,
        scrap_images,
        check_site_protocol_ssl,
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # during dev; narrow this in production
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
...
async def root_health():
    return {"status":"ok","note":"api/chat root — POST to / to call agent"}

@app.post("/" , responses={200: {"description": "Agent reply"}})
async def chat_root(payload: ChatRequest):
    # alias for same endpoint when Vercel maps file to /api/chat
...
async def _handle_chat(request: ChatRequest):
    ...
    try:
        # Runner.run is async in your earlier code; pass the message directly.
        result = await Runner.run(agent, input=request.message)
        # result.final_output should contain the agent's text
        return {"reply": result.final_output}
    except Exception as e:
